...
X_25_2f_train=np.load('/home/rahul/ICML_IntGPT/find_model/new_data/Data_25_2f.npy')[train_index,:]
Y_25_2f_train=np.load('/home/rahul/ICML_IntGPT/find_model/new_data/Loss_25_2f.npy')[train_index,0:1]

X_train=X_25_2f_train
Y_train=Y_25_2f_train
# Y_train=np.concatenate((Y_train,Y_25_sq5f_train),axis=0)
X_25_2f_test=np.load('/home/rahul/ICML_IntGPT/find_model/new_data/Data_25_2f.npy')[test_index,:]
Y_25_2f_test=np.load('/home/rahul/ICML_IntGPT/find_model/new_data/Loss_25_2f.npy')[test_index,0:1]
...
for i in range(46,47):
    for p in range(1,6):
        ...
        reg = model.fit((X_train[:,21:24]), Y_train[:,0:1])
        mse_p.append(mean_squared_error(Y_test[:,0:1], reg.predict((X_test[:,21:24]))))
plt.plot(x,mse_p)
...
regression = final_model.fit((X_train[:,21:24]), Y_train[:,0:1])
print(regression.steps[1][1].coef_,regression.steps[1][1].intercept_)
print(mean_squared_error(Y_test[:,0:1], regression.predict((X_test[:,21:24]))))